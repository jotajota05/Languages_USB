...
moveToWall :: Robot ()
moveToWall = while (isnt blocked) move
...
drawLine w c p1 p2
   = drawInWindow w (withColor c (line p1 p2))
...
spaceWait w
  = do k <- getKey w
       if (keyToChar k == ' ') then return ()
                               else spaceWait w

runRobot :: Robot () -> RobotState -> Grid -> IO ()
runRobot (Robot sf) s g
  = runGraphics $
    do w <- openWindowEx "Willy's World" (Just (0,0)) (xWin,yWin) DoubleBuffered Nothing
       drawGrid w g
       drawCoins w s
       spaceWait w
       sf s g w
...
treasureHunt = do
  penDown 
  loop 1
  where loop n = cond blocked findDoor $
                 do turnRight
                    moven n
...
spaceClose w = 
  do k <- getKey w
     if (keyToChar k == ' ') || (keyToChar k == '\x0') then closeWindow w
                                                       else spaceClose w
